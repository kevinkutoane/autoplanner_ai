...
class FakeTokenTracker extends TokenTracker {
  @override
  Future<void> log({required String action, required AIResponse response, bool success = true}) async {}
  @override
  void guardRateLimit() {}
}
...
class FakeAppMonitorService extends AppMonitorService {
  ...
  @override
  Future<void> logAIError({required String action, required String message, String? detail}) async {
    lastAction = action;
    lastMessage = message;
    lastDetail = detail;
    logCount++;
...
void main() {
  ...
  group('AIService Robustness (Manual Mocks)', () {
    ...
    test('logs AI error when JSON decoding fails', () async {
      fakeProvider.responseText = '[{"title": "incomplete"}]'; // Valid wrap, but I want invalid JSON
      ...
      fakeProvider.responseText = 'Some text [{"title": "bad" } broken ] more text';
      await aiService.parseTasks('input');
    ...
    test('skips malformed items (invalid types) within a valid JSON array', () async {
